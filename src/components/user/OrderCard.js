...
import { FaHourglassHalf, FaShippingFast, FaCheckCircle, FaTimesCircle, FaUndo } from 'react-icons/fa';

const statusConfig = {
  Pending: { bg: "bg-yellow-200", text: "text-yellow-700", icon: <FaHourglassHalf /> },
  Processing: { bg: "bg-blue-200", text: "text-blue-700", icon: <FaHourglassHalf /> },
  Shipped: { bg: "bg-purple-200", text: "text-purple-700", icon: <FaShippingFast /> },
  Delivered: { bg: "bg-green-200", text: "text-green-700", icon: <FaCheckCircle /> },
  Cancelled: { bg: "bg-red-200", text: "text-red-700", icon: <FaTimesCircle /> },
  Returned: { bg: "bg-gray-200", text: "text-gray-700", icon: <FaUndo /> },
};
...
const OrderCard = ({
  ...
  const currentStatus = statusConfig[itemStatus] || statusConfig["Pending"];  
  const onClose = () => {
    setOpen(false);
  };
  const handleCancel = () => {
  ...
  return (
    <div className="mt-2 border rounded-md flex items-center h-40">
      ...
      <div className="text-right space-y-2 flex-1 pr-4 h-full flex flex-col justify-center">
        ...
        <span className={`text-sm ${currentStatus.text} flex items-center space-x-1`}>
        {currentStatus.icon}
        <span>{itemStatus === "Active" ? "Pending" : itemStatus}</span>
      </span>
        </div>
